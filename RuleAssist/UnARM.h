...
protected:
	...
	double obfunc(int c, int w, double alpha=0.5) {
		if (c+w == 0 || MSize == 0)
			return 0;
		double p = c*1.0/(c+w);
		double r = c*1.0/MSize;
		return 1/((alpha)*1/p+(1-alpha)*1/r);
	}
	/*double obfunc(int c, int w, double alpha=0.5) {
		return c-w*1.0/2;
		return c-w;
	...
	void sortByQuality(vector<vector<CAttrRule*> >& parsNon, vector<int>& M, vector<int>& D, double alpha = 0.5);
...
public:
	...
	void searchLocalOp(ResultSet& resultSet, double alpha=0.5);
	double getOp() {return op;}
	virtual ~UnARM(void);
	/***********   Test *****************/
public:
	bool elim;
	virtual void searchTestObFunc(ResultSet& resultSet, double alpha) {}
	void testRedundancy();
	void testAccuracy(ResultSet& trainResultSetOp, ResultSet& trainResultSetLocalOp);
	void testObFunc(ResultSet& trainResultSetOp, ResultSet& trainResultSetLocalOp, double alpha);
	void testAllEfficiency();

...
void UnARM::eliminateRedundancyOp(UAttrRule* uar, vector<CAttrRule*>& cars, const vector<int>& M, const vector<int>& D, vector<CAttrRule*>& parNon, bool verbose)
{
	...
	CIXD.construct(cars, tableSimValues.columnAllSimValues(pos), D);
	...
	CIXD.clear();
	CIXD.construct(parNon1, tableSimValues.columnAllSimValues(pos), D);
	CIXM.construct(parNon1, tableSimValues.columnAllSimValues(pos), M);
	...
	if (parNon1[0]->getSimFunc() == 1 && parNon1[0]->getThreshold() == 555)
	{
		simfunc = -2;
	}
...
void UnARM::sortByQuality(vector<vector<CAttrRule*> >& parsNon, vector<int>& M, vector<int>& D, double alpha)
{
	for (unsigned i=0; i<parsNon.size(); ++i)
	{
		for (unsigned j=0; j<parsNon[i].size(); ++j)
		{
			...
			int c=0, w=0;
			satisfy(ar, M, D, c, w);
			ar->setQuality(obfunc(c,w,alpha));
		}
		sort(parsNon[i].begin(), parsNon[i].end(), cmpQuality);
	}
}
void UnARM::searchLocalOp(ResultSet& resultSet, double alpha)
{
...
void UnARM::testAccuracy(ResultSet& trainResultSetOp, ResultSet& trainResultSetLocalOp)
{
	ResultSet testResultSetOp;
	search(testResultSetOp);
	int c=0, w=0;
	initResultMap(testResultSetOp);
	satisfy(trManager.getTRSet(), trainData.getMatchingTP(), trainData.getNonMatchingTP(), c,w);
	cout << "Optimal" << "\t" << "c " << c << "\t" << "w " << w << "\t" << "obfunc " << obfunc(c,w) << endl;


	c=0, w=0;
	initResultMap(trainResultSetOp);
	satisfy(trManager.getTRSet(), trainData.getMatchingTP(), trainData.getNonMatchingTP(), c,w);
	cout << "TestOp " << "\t" << "c " << c << "\t" << "w " << w << "\t" << "obfunc " << obfunc(c,w) << endl;

	c=0, w=0;
	initResultMap(trainResultSetLocalOp);
	satisfy(trManager.getTRSet(), trainData.getMatchingTP(), trainData.getNonMatchingTP(), c,w);
	cout << "TestLOP" << "\t" << "c " << c << "\t" << "w " << w << "\t" << "obfunc " << obfunc(c,w) << endl;

	ResultSet exactResultSet = trainResultSetOp;
	for (unsigned i=0; i<exactResultSet.size(); ++i)
		exactResultSet[i].threshold = 1;
	c=0, w=0;
	initResultMap(exactResultSet);
	satisfy(trManager.getTRSet(), trainData.getMatchingTP(), trainData.getNonMatchingTP(), c,w);
	cout << "TestExa" << "\t" << "c " << c << "\t" << "w " << w << "\t" << "obfunc " << obfunc(c,w) << endl;


}

void UnARM::testObFunc(ResultSet& trainResultSetOp, ResultSet& trainResultSetLocalOp, double alpha)
{
	ResultSet testResultSetOp;
	searchTestObFunc(testResultSetOp, alpha);
	int c=0, w=0;
	initResultMap(testResultSetOp);
	satisfy(trManager.getTRSet(), trainData.getMatchingTP(), trainData.getNonMatchingTP(), c,w);
	cout << "Optimal" << "\t" << "c " << c << "\t" << "w " << w << "\t" << "obfunc " << obfunc(c,w,alpha) << endl;


	c=0, w=0;
	initResultMap(trainResultSetOp);
	satisfy(trManager.getTRSet(), trainData.getMatchingTP(), trainData.getNonMatchingTP(), c,w);
	cout << "TestOp " << "\t" << "c " << c << "\t" << "w " << w << "\t" << "obfunc " << obfunc(c,w,alpha) << endl;

	c=0, w=0;
	initResultMap(trainResultSetLocalOp);
	satisfy(trManager.getTRSet(), trainData.getMatchingTP(), trainData.getNonMatchingTP(), c,w);
	cout << "TestLOP" << "\t" << "c " << c << "\t" << "w " << w << "\t" << "obfunc " << obfunc(c,w,alpha) << endl;
	...
	initResultMap(exactResultSet);
	satisfy(trManager.getTRSet(), trainData.getMatchingTP(), trainData.getNonMatchingTP(), c,w);
	cout << "TestExa" << "\t" << "c " << c << "\t" << "w " << w << "\t" << "obfunc " << obfunc(c,w,alpha) << endl;


}
void UnARM::testAllEfficiency()
{
	ResultSet resultSet;
	cout << "elimredu" << "\t";
	elim = true;
	...
	cout << "clock " << (endClock-1.0*beginClock)/CLOCKS_PER_SEC << "\t" <<  "time " << endTime-beginTime << endl;
...
void UnARM::ouputTP(const TuplePair& tp)
{
	cout << (tp.type ? '+':'-') << endl;
	assert(tp.tuple1.size() == tp.tuple2.size());
	for (unsigned i=0; i<tp.tuple1.size(); ++i)
	{
		cout << tp.tuple1[i] << "\t";
	}
	cout << endl;
	for (unsigned i=0; i<tp.tuple2.size(); ++i)
	{
		cout << tp.tuple2[i] << "\t";
	}
	cout << endl;
...
void UnARM::satisfyVerbose(const vector<TupleRule>& trs, const vector<int>& M, const vector<int>&D,  int& c, int& w)
{
	for (unsigned i=0; i<M.size(); ++i)
	{
		bool flag = false;
		...
		if (!flag)
			ouputTP(trainData.getTuplePair(M[i]));
	}
	for (unsigned i=0; i<D.size(); ++i)
	{
		for (unsigned j=0; j<trs.size(); ++j)
		{
			...
			{
				++w;
				ouputTP(trainData.getTuplePair(D[i]));
				break;
			}
		}
	}
